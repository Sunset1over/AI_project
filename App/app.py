...
song_library = pd.read_csv('C:/Users/artem/PycharmProjects/AI_project/App/song_library.csv', na_filter=False)
...
def song_recommender(song_name1, song_name2):
    try:
        ...
        text_vec1 = song_vectorizer.transform(song_library[song_library['name'] == str(song_name1)]['genres']).toarray()
        text_vec2 = song_vectorizer.transform(song_library[song_library['name'] == str(song_name2)]['genres']).toarray()

        # Create vector from numerical columns for given songs
        num_vec1 = song_library[song_library['name'] == str(song_name1)][num_cols].to_numpy()
        num_vec2 = song_library[song_library['name'] == str(song_name2)][num_cols].to_numpy()
        ...
        for index, row in song_library.iterrows():
            ...
            text_vec_other = song_vectorizer.transform(song_library[song_library['name'] == name]['genres']).toarray()

            # Create vector from numerical columns for other songs
            num_vec_other = song_library[song_library['name'] == name][num_cols].to_numpy()
        ...
        recommended_songs['Listen on Spotify'] = ['https://open.spotify.com/track/{}'.format(id) for id in
        ...
        error_msg = '{} or {} not found in songs library.'.format(song_name1, song_name2)
        return error_msg


# Artist recommender
# Read artist library data file
artist_library = pd.read_csv('C:/Users/artem/PycharmProjects/AI_project/App/artist_library.csv', na_filter=False)
...
def artist_recommender(artist_name):
    try:
        ...
        text_vec1 = artist_vectorizer.transform(
            artist_library[artist_library['name'] == str(artist_name)]['genres']).toarray()
        ...
        # Sort DataFrame based on "similarity" column
        artist_library.sort_values(by=['similarity', 'popularity', 'followers'], ascending=[False, False, False],
                                   inplace=True)
        ...
        recommended_artists['View on Spotify'] = ['https://open.spotify.com/artist/{}'.format(id) for id in
                                                  recommended_artists['id']]
...
# Spotify recommender
def spotify_recommender(song_name1, song_name2):
    try:
        # Get DataFrame of recommended songs using song_recommender() function
        recommended_songs = song_recommender(song_name1, song_name2)
        ...
        artists1 = song_library[song_library['name'] == str(song_name1)]['artists'].values[0].split(',')
        artists2 = song_library[song_library['name'] == str(song_name2)]['artists'].values[0].split(',')

        # Combine contributing artists
        ...
        # If given song is not found in song library then return error message
        error_msg = '{} not found in songs library.'.format(song_name1)
        return [error_msg]
...
set_background('C:/Users/artem/PycharmProjects/AI_project/background.png')

# Page title
st.title('Spotify Recommendation System')

# Get input from user
song_name1 = st.text_input('Song name 1: ')
song_name2 = st.text_input('Song name 2: ')

if song_name1 and song_name2:
    # Call spotify_recommender() function with given song names
    return_values = spotify_recommender(song_name1, song_name2)
    # If there is only 1 return value then display error message
    if len(return_values) == 1:
        st.write('')
        st.write('Oops! {} was not found in the songs library.'.format(song_name1))
    ...
    # If return_val1 is not False then display recommended songs and artists
        ...
        # Rename columns
        recommended_songs.rename(columns={'name': 'Name', 'artists': 'Artists', 'release_year': 'Release Year'},
                                 inplace=True)
        recommended_artists.rename(columns={'name': 'Name', 'followers': 'Followers', 'popularity': 'Popularity'},
                                   inplace=True)
